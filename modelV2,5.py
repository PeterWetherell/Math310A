...
model.compile(optimizer=Adam(learning_rate=initial_learning_rate), loss=ProjectUtils.log_spectral_distancell)
...
for seg_num in range(num_segments):
    ...
    for sub_seg_num in range(num_sub_segments):
        ...
        for i in range(1, data_length - height, speedup): # Remove the two padding frames
            ...
            if ((data_length - height + 1 - 2) // speedup <= input_index):
                continue
    ...
    if (seg_num % num_segments_per_save == 0 and seg_num != 0):
        model.save(f'./Models2/deonoiserV2,5-{seg_num // num_segments_per_save}-0.keras')
model.save(f'./Models2/deonoiserV2,5-{seg_num // num_segments_per_save}-{seg_num % num_segments_per_save}.keras')